#!/usr/bin/env python3
"""Reacquire and verify one bounded production Security publication."""
...
class NoRedirect(urllib.request.HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):  # noqa: ANN001
        raise PublicationError(f"Security origin returned forbidden redirect HTTP {code}")
...
def fetch(opener: urllib.request.OpenerDirector, url: str, maximum: int) -> bytes:
    request = urllib.request.Request(url, headers={
        ...
        "User-Agent": "universal-agent-plugins-security-observer/1",
    })
    try:
        with opener.open(request, timeout=20) as response:
            if response.status != 200:
                raise PublicationError(f"Security origin returned HTTP {response.status}")
            body = response.read(maximum + 1)
    except PublicationError:
        raise
    except (OSError, urllib.error.URLError) as error:
        raise PublicationError(f"Security origin fetch failed: {error}") from error
    if len(body) > maximum:
        raise PublicationError(f"Security origin response exceeds {maximum} bytes")
    return body
...
def observe_once(origin: str, trusted_keys: Path, minimum_sequence: int) -> dict[str, object]:
    ...
    if canonical_json(latest) != latest_body:
        raise PublicationError("Security latest pointer is not canonical JSON")
    validate_with_schema(latest, LATEST_SCHEMA)
    if latest["sequence"] < minimum_sequence:
        raise PublicationError("Security production sequence is below the required floor")

    with tempfile.TemporaryDirectory(prefix="uap-security-observe-") as temporary:
        feed = Path(temporary)
        ...
        if now < parse_timestamp(snapshot["generated_at"], "generated_at"):
            raise PublicationError("Security production snapshot is not active yet")
        if now >= parse_timestamp(snapshot["expires_at"], "expires_at"):
            raise PublicationError("Security production snapshot is expired")
        ...
        return {
            ...
            "subjects": snapshot["coverage"]["subjects"],
            "checked": snapshot["coverage"]["checked"],
    ...
    for attempt in range(args.attempts):
        try:
            ...
            if attempt + 1 < args.attempts:
                time.sleep(5)
    print(f"Security observation failed after {args.attempts} attempts: {error}", file=sys.stderr)
    return 1
