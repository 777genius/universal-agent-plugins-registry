...
test('keeps each plugin selector compact above a two-line command', async ({ page }) => {
  ...
  await expect(command.locator('code')).toHaveText(/npx universal-agent-plugins add discovery:example\/portable\/\/agent-plugin\s+--target cursor/)
  ...
  await expect(page.getByRole('checkbox', { name: /Claude Code/ })).toBeEnabled()
  await expect(page.getByRole('checkbox', { name: /Gemini CLI/ })).toBeEnabled()
  await expect(page.getByRole('checkbox', { name: /OpenCode/ })).toBeEnabled()
  await expect(page.getByRole('checkbox', { name: /Cline/ })).toBeEnabled()
  await expect(page.getByRole('checkbox', { name: /Windsurf/ })).toBeEnabled()
  await page.keyboard.press('Escape')
  await expectNoHorizontalOverflow(page)
  expect(failures).toEqual([])
})
