...
'''
    ...
    def test_catalog_gate_attests_both_canonical_subjects_and_exact_oidc_invocation(self) -> None:
        ...
        self.assertEqual(workflow["env"]["AGENTPLUGINS_VERSION"], "0.1.51")
        self.assertEqual(workflow["env"]["AGENTPLUGINS_COMMIT"], "aa9e03e4e6bc9eb044aedde8be1d1ff4ea514a2c")
        readiness_tree = ast.parse((ROOT / "scripts/catalog_publication_readiness.py").read_text())
        readiness_cli = ast.literal_eval(next(
            node.value for node in readiness_tree.body
            if isinstance(node, ast.Assign)
            and any(isinstance(target, ast.Name) and target.id == "CLI" for target in node.targets)
        ))
        self.assertEqual(readiness_cli["version"], workflow["env"]["AGENTPLUGINS_VERSION"])
        self.assertEqual(readiness_cli["tag"], f"agentplugins-v{workflow['env']['AGENTPLUGINS_VERSION']}")
        self.assertEqual(readiness_cli["commit"], workflow["env"]["AGENTPLUGINS_COMMIT"])
        self.assertIn(readiness_cli["binary_digest"].removeprefix("sha256:"), commands(producer))
  ...
  --completed two-lane-readiness/two-lane-readiness.json""")
    ...
    def test_discovery_index_is_lkg_protected_and_preserves_promoted_directory(self) -> None:
        ...
        publisher_identity = next(
            step for step in publisher_preflight["steps"]
            if step.get("name") == "Record non-secret publisher integration identity"
        )
        self.assertEqual(
            publisher_identity["env"]["PUBLISHER_APP_ID"],
            "${{ secrets.DISCOVERY_PUBLISHER_APP_ID }}",
        )
        self.assertIn('int(os.environ["PUBLISHER_APP_ID"])', publisher_identity["run"])
    ...
    def test_security_index_binds_exact_discovery_subjects_and_preserves_directory(self) -> None:
        ...
        self.assertIn("python3 -m scripts.security_index", scan_body)
        self.assertNotIn("python3 scripts/security_index.py", scan_body)
